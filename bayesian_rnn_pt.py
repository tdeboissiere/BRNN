...
class PTBModel(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_size, num_layers,
                 dropout=0.5, log_sigma1=-1.0, log_sigma2=-7.0, prior_pi=0.25, var_mode="BBB"):
        ...
        self.encoder = models.BayesEmbedding(vocab_size, hidden_size,
                                             log_sigma1=-1.0, log_sigma2=-7.0, prior_pi=0.25, var_mode=var_mode)
        self.bayeslstm = models.BayesLSTM(hidden_size, hidden_size,
                                          num_layers=2, log_sigma1=-1.0, log_sigma2=-7.0, prior_pi=0.25, var_mode=var_mode)
        self.linear = models.BayesLinear(hidden_size, vocab_size,
                                         log_sigma1=-1.0, log_sigma2=-7.0, prior_pi=0.25, var_mode=var_mode)
...
def run_epoch(model, criterion, train_data, corpus, lr, epoch):
    ...
    for batch, i in enumerate(tqdm(range(0, train_data.size(0) - 1, args.num_steps))):
        ...
        if model.var_mode == "BBB":
            ...
            # total_kl_loss += np.array([0])  # dummy value
        else:
...
def run(args):
    ...
    # Create model
    model = PTBModel(ntokens, args.hidden_size, args.num_layers,
                     dropout=args.dropout, log_sigma1=args.log_sigma1,
                     log_sigma2=args.log_sigma2, prior_pi=args.prior_pi, var_mode=args.var_mode)
    print(model)
